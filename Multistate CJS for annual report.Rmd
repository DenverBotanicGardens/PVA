...
```{r}
load("Q:/Research/All_Projects_by_Species/Astragalus SPECIES/Astragalus_microcymbus/2024_forDBG0077/multi_20241118.rdata")

asmi.modelsMulti20241031
```
...
asmiMultiCebolla.ddl$S <- asmiMultiCebolla.ddl$S %>%
  left_join(WinterPPTCebolla, join_by("time" == "Prev12", 
                              "Plot" == "Plot"))
...
asmiMultiCebolla.ddl$S <- asmiMultiCebolla.ddl$S %>%
  left_join(SummerMaxCebolla, join_by("time" == "Prev12",
                              "Plot" == "Plot"))
...
asmiCebolla <- function(){
  S.stratumTimeSite = list(formula = ~ stratum:time:Site)

  p.strata = list(formula = ~ stratum,
               fixed = list(index = p.Cebollaindices, value = p.Cebollavalues))
  Psi.stratumTimeSite = list(formula = ~ -1 + stratum:tostratum:time:Site)
...
Psi.tableCebolla <- get.real(asmi.modelCebolla$S.stratumTimeSite.p.strata.Psi.stratumTimeSite, "Psi", vcv = TRUE)
...
PsivaluesSite <- PsivaluesCebolla %>%
  ...
  group_by(Plot,group,stratum,tostratum) %>%
...
### WHY?!?! Why do confidence intervals include NAs??? Because too close to zero?? they tend to switch back and forth between veg and reprodutive? 
TM.CebollaMid <- TransitionMatrix(PsivaluesSite[PsivaluesSite$Plot == 1,], vcv.real = Psi.table$vcv.real)
TM.CebollaMid <- TransitionMatrix(PsivaluesSite[PsivaluesSite$Plot == 16 & PsivaluesSite$time == "2015",], vcv.real = Psi.table$vcv.real)

## Transitions
Psi.strataSite <- get.real(asmi.modelCebolla$S.stratumTimeSite.p.strata.Psi.stratumTimeSite, "Psi", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  filter(fixed != "Fixed") %>%
  group_by(group, stratum) %>%
  summarise(Psi.estimates = mean(estimate),
            Psi.lcl = mean(lcl),
            Psi.ucl = mean(ucl))

## Survival
S.strataSite <- get.real(asmi.modelCebolla$S.stratumTimeSite.p.strata.Psi.stratumTimeSite, "S", se = TRUE) %>%  
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  filter(fixed != "Fixed") %>%
  group_by(group, stratum) %>%
  summarise(S.estimates = mean(estimate),
            S.lcl = mean(lcl),
            S.ucl = mean(ucl))
...
asmiMultiSouthBeaver.ddl$S$Psience <-  ifelse(asmiMultiSouthBeaver.ddl$S$time %in% PlotSiteYrFence$year[PlotSiteYrFence$Psienced == "y"] &
                               asmiMultiSouthBeaver.ddl$S$Plot %in% PlotSiteYrFence$plot[PlotSiteYrFence$Psienced == "y"],1,0)
asmiMultiSouthBeaver.ddl$S$Psience <- as.factor(asmiMultiSouthBeaver.ddl$S$Psience)

asmiMultiSouthBeaver.ddl$p$Psience <-  ifelse(asmiMultiSouthBeaver.ddl$p$time %in% PlotSiteYrFence$year[PlotSiteYrFence$Psienced == "y"] &
                               asmiMultiSouthBeaver.ddl$p$Plot %in% PlotSiteYrFence$plot[PlotSiteYrFence$Psienced == "y"],1,0)
asmiMultiSouthBeaver.ddl$p$Psience <- as.factor(asmiMultiSouthBeaver.ddl$p$Psience)

  
asmiMultiSouthBeaver.ddl$Psi$Psience <-  ifelse(asmiMultiSouthBeaver.ddl$Psi$time %in% PlotSiteYrFence$year[PlotSiteYrFence$Psienced == "y"] &
                               asmiMultiSouthBeaver.ddl$Psi$Plot %in% PlotSiteYrFence$plot[PlotSiteYrFence$Psienced == "y"],1,0)
asmiMultiSouthBeaver.ddl$Psi$Psience <- as.factor(asmiMultiSouthBeaver.ddl$Psi$Psience)
...
p.SouthBeaverindices <- c(pindex, p1.SouthBeaverindices)
...
asmiTimeFence <- function(){
  S.dot = list(formula = ~ stratum:time:Site:fence)

  p.strata = list(formula = ~ stratum,
               fixed = list(index = p.indices, value = p.values))
  Psi.dot = list(formula = ~ -1 + stratum:tostratum:time:Site:fence)
...
asmi.modelFenceCreek <- asmiTimeFenceCreek()
...
```{r}
asmiTimeFenceCreek <- function(){
  S.dot = list(formula = ~ stratum:time:Creek:fence)

  p.strata = list(formula = ~ stratum,
               fixed = list(index = p.indices, value = p.values))
  Psi.dot = list(formula = ~ -1 + stratum:tostratum:time:Creek:fence)
...
save(asmi.modelFenceCreek, file = "Q:/Research/All_Projects_by_Species/Astragalus SPECIES/Astragalus_microcymbus/2024_forDBG0077/multi_timecreekfence.rdata")

```

#### 
```{r}
load("Q:/Research/All_Projects_by_Species/Astragalus SPECIES/Astragalus_microcymbus/2024_forDBG0077/multi_timecreekfence.rdata")

asmi.modelFenceCreek


surv.table <- get.real(asmi.modelFenceCreek$S.dot.p.strata.Psi.dot, "S", se = TRUE)

surv.Creek <- surv.table %>%
  filter(fixed != "Fixed") %>%
  left_join(plotSiteCreek) %>%
  group_by(Creek, stratum) %>%
  summarise(estimates = mean(estimate),
...
psi.table <- get.real(asmi.modelFenceCreek$S.dot.p.strata.Psi.dot, "Psi", se = TRUE)
...
### remain in strata by subtraction 
psi.Creekremain <- psi.Creek %>%
  mutate(tostratum = case_when(tostratum == "V" ~ "R",
                               tostratum == "R" ~ "V")) %>%
  mutate(estimates = 1-estimates,
         lcl = 1-lcl,
         ucl = 1-ucl) 

psi.Creekalltrans <- psi.Creek %>%
  bind_rows(psi.Creekremain)


## Is this the same as TransitionMatrix???
psi.Creekalltrans %>%
  rename(S.estimates = estimates,
        S.lcl = lcl,
        S.ucl = ucl) %>%
  left_join(surv.Creek) %>%
  mutate(MPM = S.estimates * estimates)
...
asmi.modelFenceCreek$model.table
## S ~ stratum:time:Creek:fence
## p ~ stratum
## Psi ~ stratum:tostratum:time:Creek:fence


Psi.table <- get.real(asmi.modelFenceCreek$S.dot.p.strata.Psi.dot, "Psi", vcv = TRUE)

save(Psi.table, file = "C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/hackathon/GitPVA/datasets/Psi_tableFenceCreekStrata.Rdata")
...
PsivaluesCreek <- Psivalues %>%
  left_join(plotSiteCreek) %>%
  filter((Creek == "Cebolla" & Time > 18) | Creek == "SouthBeaver") %>%
  group_by(Creek, time) %>%
  mutate(Creek.estimate = mean(estimate),
            Creek.se = mean(se),
            Creek.lcl = mean(lcl),
            Creek.ucl = mean(ucl))
