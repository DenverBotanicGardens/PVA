...
<http://www.phidot.org/forum/viewtopic.php?f=21&t=4158>   
...
### AMDEND THIS FOR STAYING IN ASMI!!
```{r}
library(RMark)
library(msm)
data(mstrata)
...
```{r}
## get covariance between survival and transitions
?covariate.predictions


## Delta method for z by hand:
## 1.  Get variance of product z=xy  
## 2.  then compute log(z/(1-z)) and apply delta method to compute variance of z, conf interval, back transform end points to get confidence interval for z

## Notes:
### get.real gives you one parameter
### covariate.predictions to extract both S and Psi together and get covariances between S and Psi. (even if you don't have covariates)
### 1. pride the set of indicies for teh parameters you want, it provies the real estimates and the var-cov matrix. 
### formula for staying (remaining in a strata) is 1-sum of moving to other strata. 
...
```{r}
p.table <- get.real(multi.marktest, "p", vcv = FALSE)
