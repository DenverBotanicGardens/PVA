...
load(paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Astragalus-microcymbus/Astragalus-microcymbus_Data/stage-fate", currentyr,".Rdata", sep=""))
...
asmiCebolla <- function(){
  ...
  S.stratumSiteWinter = list(formula = ~ stratum:ppt_winter:Site) 
  S.stratumSiteSummer = list(formula = ~ stratum:tmax_springSummer:Site) 
  S.stratumSiteSummerWinter = list(formula = ~ stratum:ppt_winter:tmax_springSummer:Site) 
  ...
  Psi.stratumSitewinter = list(formula = ~ -1 + stratum:tostratum:ppt_winter:Site)
  Psi.stratumSitesummer = list(formula = ~ -1 + stratum:tostratum:tmax_springSummer:Site)
  Psi.stratumSitewintersummer = list(formula = ~ -1 + stratum:tostratum:tmax_springSummer:ppt_winter:Site)
...
p.strataSite <- get.real(asmi.modelCebolla$S.stratumSiteWinter.p.strata.Psi.stratumSitewinter, "p", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  dplyr::rename(p.estimates = estimate,
         p.lcl = lcl,
         p.ucl = ucl) %>%
  dplyr::select(group, Plot, time, stratum,  p.estimates, p.lcl, p.ucl)


## Transitions
Psi.strataSiteWinter <- get.real(asmi.modelCebolla$S.stratumSiteWinter.p.strata.Psi.stratumSitewinter, "Psi", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  dplyr::rename(Psi.estimates = estimate,
         Psi.lcl = lcl,
         Psi.ucl = ucl) %>%
  dplyr::select(group, Plot, time, stratum, tostratum, Psi.estimates, Psi.lcl, Psi.ucl) %>%
  mutate(Prev12 = as.numeric(as.character(time)) +1 ) %>%
  left_join(asmi.season.wide, by = c("Prev12" = "Prev12", "Plot" = "Plot")) 

stratatostratalabels <- c("R" = "Vegetative to Reproductive", "V" = "Reproductive to Vegetative")

CC.Phi <- ggplot(Psi.strataSiteWinter, aes(ppt_winter, Psi.estimates, color = Plot)) +
  geom_point(position = position_dodge(width = .1)) +
  geom_errorbar(aes(ymin = Psi.lcl, ymax = Psi.ucl),position = position_dodge(width = .1))+
...
S.strataSiteWinter <- get.real(asmi.modelCebolla$S.stratumSiteWinter.p.strata.Psi.stratumSitewinter, "S", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  dplyr::rename(S.estimates = estimate,
         S.lcl = lcl,
         S.ucl = ucl) %>%
  dplyr::select(group, Plot, time, stratum, S.estimates, S.lcl, S.ucl) %>%
  mutate(Prev12 = as.numeric(as.character(time)) +1 ) %>%
  left_join(asmi.season.wide, by = c("Prev12" = "Prev12", "Plot" = "Plot")) 

CC.S <- ggplot(S.strataSiteWinter, aes(ppt_winter, S.estimates, color = Plot)) +
  geom_point(position = position_dodge(width = .1)) +
  geom_errorbar(aes(ymin = S.lcl, ymax = S.ucl),position = position_dodge(width = .1))+
  facet_wrap(~stratum)+
  theme_bw() +
  ylab("Survival rate")+
  scale_color_manual("Site", values = c("darkblue","skyblue"),
...
Psi.strataSite <- get.real(asmi.modelCebolla$S.stratumSite.p.strata.Psi.stratumSite, "Psi", se = TRUE) %>%
  ...
  dplyr::rename(Psi.estimates = estimate,
         Psi.lcl = lcl,
         Psi.ucl = ucl) %>%
  dplyr::select(group, Plot, stratum, tostratum, Psi.estimates, Psi.lcl, Psi.ucl)
...
S.strataSite <- get.real(asmi.modelCebolla$S.stratumSite.p.strata.Psi.stratumSite, "S", se = TRUE) %>%  
  ...
  dplyr::rename(S.estimates = estimate,
         S.lcl = lcl,
         S.ucl = ucl) %>%
  dplyr::select(group, Plot, stratum, S.estimates, S.lcl, S.ucl)
...
South Beaver     
Takes a long time, run on separate computer    
```{r, eval = FALSE}


asmiSouthBeaver <- function(){
  S.stratumSiteFence = list(formula = ~ stratum:Site:fence)
  S.stratumSiteWinter = list(formula = ~ stratum:ppt_winter:Site:fence) 
  S.stratumSiteSummer = list(formula = ~ stratum:tmax_springSummer:Site:fence) 
  S.stratumSiteSummerWinter = list(formula = ~ stratum:ppt_winter:tmax_springSummer:Site:fence) 
  ...
  Psi.stratumSitewinter = list(formula = ~ -1 + stratum:tostratum:ppt_winter:Site:fence)
  Psi.stratumSitesummer = list(formula = ~ -1 + stratum:tostratum:tmax_springSummer:Site:fence)
  Psi.stratumSitewintersummer = list(formula = ~ -1 + stratum:tostratum:tmax_springSummer:ppt_winter:Site:fence)
...
p.strataSiteSBC <- get.real(asmi.modelSouthBeaver$S.stratumSiteSummerWinter.p.strata.Psi.stratumSiteFence, "p", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  dplyr::rename(p.estimates = estimate,
         p.lcl = lcl,
         p.ucl = ucl) %>%
...
## What is wrong?? 
## Transitions
Psi.strataSiteWinterSBC <- get.real(asmi.modelSouthBeaver$S.stratumSiteSummerWinter.p.strata.Psi.stratumSiteFence, "Psi", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  dplyr::rename(Psi.estimates = estimate,
         Psi.lcl = lcl,
         Psi.ucl = ucl) %>%
  ...
  mutate(time = as.numeric(as.character(time)),
         group = as.integer(as.character(group))) %>%
  ## Maybe because we added 19 a year later than 26 and 5?? 
  left_join(PlotSiteYrFence, join_by("group" == "plot", "time" == "year")) %>%
  mutate(fenced = case_when((group %in% c(512,300) & time == 2006) ~ "y",
                            TRUE ~ fenced)) %>%
  # mutate(Prev12 = as.factor(as.numeric(as.character(time)) + 1)) %>%
  mutate(Prev12 = as.numeric(as.character(time)) +1 ) %>%
  left_join(asmi.season.wide, by = c("Prev12" = "Prev12", "Plot" = "Plot")) 
...
# Psi.strataSiteWinterSBC %>%
#   filter(group %in% c(512,300) & time == 2006)

stratatostratalabels <- c("R" = "Vegetative to Reproductive", "V" = "Reproductive to Vegetative")

## I think one of the Site 19s that should be marked Yes fenced is still a No
SBC.Phi <- ggplot(Psi.strataSiteWinterSBC, aes(fenced, Psi.estimates, color = as.factor(site.x))) + ## Was site 
...
S.strataSiteWinterSBC <- get.real(asmi.modelSouthBeaver$S.stratumSiteSummer.p.strata.Psi.stratumSiteFence, "S", se = TRUE) %>%
  ...
  mutate(Prev12 = as.numeric(as.character(time)) +1 ) %>%
  left_join(asmi.season.wide, by = c("Prev12" = "Prev12", "Plot" = "Plot")) %>%
  dplyr::mutate(tmax_springSummer_bin = cut(tmax_springSummer, breaks = 2)) %>%
  dplyr::mutate(ppt_winter_bin = cut(ppt_winter, breaks = 2))

S.strataSiteWinterSBC %>%
  filter(group %in% c(512,300) & fenced == "y")


SBC.S <-
S.strataSiteWinterSBC %>%
  # group_by(site.x, ppt_winter_bin,stratum) %>%  #tmax_springSummer_bin, 
  # dplyr::mutate(S.est = mean(S.estimates),
  #                  S.l = mean(S.lcl),
  #                  S.u = mean(S.ucl)) %>%
ggplot(   aes(tmax_springSummer, S.estimates, color = as.factor(site.x))) +
  geom_point(position = position_dodge(width = 0.5)) +
  geom_errorbar(aes(ymin = S.lcl, ymax = S.ucl),position = position_dodge(width = 0.5))+
  theme_bw() +
  ylab("Survival rate")+
  xlab(expression("Summer maximum temperature "~ "\u00B0C"))+
  scale_color_manual("Site", values = c("darkblue","skyblue","orange","brown")) +
  facet_grid( fenced ~ stratum , scales = "free",
              labeller = labeller(fenced = c("n" = "Open",
                                             "y" = "Fenced"),
                                  stratum = c("V" = "Vegetative",
                                              "R" = "Reproductive"),
                                  ppt_winter_bin = c("(-2.23,0.982]" = "<= Avg Winter PPT",
                                                     "(0.982,4.19]" = "> Avg Winter PPT"),
                                  tmax_springSummer_bin = c("(-2.37,-0.0279]" = "<= Avg Max Summer Temp",
                                                            "(-0.0279,2.32]" = "> Avg Max Summer Temp")))
