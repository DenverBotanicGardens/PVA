...
asmiMulti.proc <- process.data(asmiMulti, begin.time = 1995, model = "Multistrata", groups = "Plot",
                               strata.labels = c("V","R"))
## Avector of strata (one for each strata) that is the tostratum that is computed by subtraction
asmiMulti.ddl <- make.design.data(asmiMulti.proc,
                                  parameters = list(Psi=list(subtract.stratum=c("V","R"))))
```
...
starttime <- Sys.Date()
###### Need fencing in all and stratum in all. Detection of reproductive will be different than vegetative
multi.mark2WinSumadditive <- mark(asmiMulti.proc, asmiMulti.ddl, begin.time = 1995, model = "Multistrata",
...
save(multi.mark2WinSumadditive, file = paste("Q:/Research/All_Projects_by_Species/Astragalus SPECIES/Astragalus_microcymbus/2024_forDBG0077/",
                               starttime, "multimarkStratumFenceWinterSummerAdditive",
                               endtime, ".rdata", sep=""))
```
