...
## Transitions
Psi.strataSite <- get.real(asmi.modelCebolla$S.stratumSiteWinter.p.strata.Psi.stratumSitewinter, "Psi", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
...
stratatostratalabels <- c("R" = "V to R", "V" = "R to V")

ggplot(Psi.strataSite, aes(WinterPPT, Psi.estimates, color = Plot)) +
...
S.strataSite <- get.real(asmi.modelCebolla$S.stratumSiteWinter.p.strata.Psi.stratumSitewinter, "S", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
...
ggplot(S.strataSite, aes(WinterPPT, S.estimates, color = Plot)) +
  geom_point(position = position_dodge(width = 1)) +
  geom_errorbar(aes(ymin = S.lcl, ymax = S.ucl),position = position_dodge(width = 1))+
  facet_wrap(~stratum, labeller = labeller(tostratum = stratatostratalabels))+
  theme_bw() +
  ylab("Survival rate")
...
save(asmi.modelSouthBeaver, file = paste(savepath, "asmimodelSouthBeaver.Rdata", sep = ""))
