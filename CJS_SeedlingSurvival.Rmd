...
intraApril2014 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnAprl14:CommentsApr14)) %>%
...
intraMay2014 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnMay12:Comments)) %>%
...
intraJune22014 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnJun14:Comments.1)) %>%
...
intraJune252014 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnJun214:Comments.2)) %>%
...
intraJul2014 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnJul14:Comments.3)) %>%
...
intraAug2014 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnAug14:Comments.4)) %>%
...
intraSept2014 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnSep14:Comments.5)) %>%
...
intraOct2014 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnOct14:Comments.6)) %>%
...
intraOct282014 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnOct2814:Comments.7)) %>%
...
intraApril2015 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnApr20_15:Comments.8)) %>%
...
intraMay112015 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnMay11_15:Comments.9)) %>%
  mutate(Day = as.POSIXlt(as.Date("2015-05-11"))$yday) %>%
  mutate(Year = 2015) %>%
  # complete(Day, nesting(Site,Plot.1,tag), fill = list(Lngth13 = 0, Fl13 = 0, Fr13 = 0, Br13 = 0)) %>%
  # group_by(Site, Plot.1) %>%
  # summarise(K.Plot = n_distinct(Day)) %>%
  rename(Plot = Plot.1) %>%
  relocate(Site, .before = tag)

intraMay262015 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnMay26_15:comments)) %>%
...
intraJune8112015 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnJune8_15:comments.1)) %>%
...
intraJune222015 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnJune22.24:comments.2)) %>%
...
intraJuly2015 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnJuly20.23_15:comments.3)) %>%
...
intraAugust2015 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnAug12.14_15:comments.4)) %>%
...
intraSept2015 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnSept1.3_15:comments.5)) %>%
...
intraOct2015 <- intra.2014_2015 %>%
  select(c(Site,Plot.1,tag,LnOct6.8_15:comments.6)) %>%
...
surveys <- list(intraApril2014,intraMay2014,intraJune22014, intraJune252014, intraJul2014, 
...
table(intra20142015$Tag, useNA = "ifany") ## 18 have tag number 0, not now
...
dataCJS <- intra %>%
  ...
  mutate(K = paste0(Year, encounter), 
         Obs = case_when(Ln > 0 & Fl == 0 & !grepl("coty", Comments) ~ 1,
                         Ln > 0 & Fl == 0 & grepl("coty", Comments) ~ 2,
                         Ln > 0 & Fl == 1 ~ 3,
                         Ln == 0 ~ 0)) %>%
...
## There are some vertical lines that shouldn't be there. Not same PlotTag in two different states on same day 
# No cotyloedons observed in 2013, we didn't know what they looked like yet
dataCJS %>%
  # filter(grepl("coty", Comments)) %>%
ggplot(  aes(Day, Obs, color = as.factor(PlotTag), linetype = as.factor(Year)))+
  geom_line()+
  # geom_jitter()+
  geom_point() +
  # facet_wrap(~Year)+
...
dataCJS[dataCJS$Obs == 2,]

## Start with 2014; some are missing a tag number. 33 don't have a Tag number

obsmodel <- dataCJS %>%
  # filter(!(Year == 2014 & Tag == 248 & Day == 279 & Ln == 7)) %>%
  distinct(Day,Year,Plot,Site,Tag,encounter, .keep_all = TRUE) %>%
  select(c(Site:Tag, encounter, Obs, Year)) %>%
  pivot_wider(names_from = encounter, values_from = Obs, values_fill = 0)

## Keys are shared for 34 rows, before using distinct
# dataCJS[3393:3394,]

save(obsmodel, file = "C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/hackathon/GitPVA/datasets/Observations3years.Rdata")

obsmodel %>%
  filter(Year == 2014)

# model_data <- list(numPlots = nrow(K_plotcomplete),
#                    Site = as.factor(encounters$Site),
#                    Plot = as.factor(encounters$Plot),
#                    Kplot = K_plotcomplete$LastK,   ## need row for each tag within
#                    nplot = K_plotcomplete$nx.plot,
#                    n = length(encounters$e),
#                    nx = sum(K_plotcomplete$K.plot),  ## have one row for each tag "*K_plotcomplete$nx.plot"
#                    e = encounters$e,
#                    data1 = dataCJS[,1],
#                    data2 = dataCJS[,2],
#                    data3 = dataCJS[,3])
...
#   distinct(Day,Year,Plot,Site,Tag,encounter, .keep_all = TRUE) %>%
#   select(c(Site:Tag, Day, Obs, Year, fence)) %>%
#   arrange(Day) %>%
#   pivot_wider(names_from = Day, values_from = Obs, values_fill = NA) ## So that a missed occasion can be picked out
  
obsmodel <- dataCJSfence %>%
  distinct(Day,Year,Plot,Site,Tag,encounter, .keep_all = TRUE) %>%
  select(c(Site:Tag, encounter, Obs, Year, fence)) %>%
  arrange(Day) %>%
  pivot_wider(names_from = encounter, values_from = Obs, values_fill = 0) 

CJS_AsMi_fence <- obsmodel %>%
  filter(Year == 2013) %>%
