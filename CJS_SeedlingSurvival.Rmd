...
table(intra20142015$Tag, useNA = "ifany") ## 18 have tag number 0
...
dataCJS <- intra %>%
  ...
  mutate(K = paste0(Year, encounter), Obs = case_when(Ln > 0 & Fl == 0 & !grepl("coty", Comments) ~ 1,
...
obsmodel <- dataCJS %>%
  ...
  select(c(Plot:Tag, encounter, Obs, Year)) %>%
