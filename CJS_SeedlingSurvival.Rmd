...
## So close, need zeros and "."s for the years when not recorded, 
# when missing in a year, can be the pattern of characters and . for that plot but all zeros
asmiRD <- obsmodel %>%
  mutate(across(Occ1:Occ10, ~ ifelse(.x > 0, 1, .x))) %>%
  unite(ch, Occ1:Occ10, sep="") %>%
  group_by(Site,Plot,Tag) %>%
  mutate(ch = paste0(ch, collapse = ""))

asmiRD[nchar(asmiRD$ch) == 30,] ## 288 individuals across all three years 
