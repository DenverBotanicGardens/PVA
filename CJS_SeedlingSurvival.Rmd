...
intraApril2014 <- intra.2014_2015 %>%
  ...
  mutate(Year = 2014) %>%
  mutate(Day = as.POSIXlt(as.Date("2014-04-20"))$yday) %>%
...
surveys <- list(intraApril2014,intraMay2014,intraJune22014, intraJune252014, intraJul2014, intraAug2014, intraSept2014, intraOct2014,
                                     intraApril2015, intraMay112015, intraMay262015, intraJune8112015, intraJune222015, intraJuly2015, intraAugust2015, intraSept2015, intraOct2015)
...
rm(intra.2014_2015);rm(intraApril2014);rm(intraApril2014);rm(intraApril2015);rm(intraMay112015);rm(intraMay2014);rm(intraMay262015);
...
```{r}
intra.2013complete[intra.2013complete$Plot == 578,]
...
K_plotcomplete ## The number of visits per plot K.plot, individuals per plot nx.plot, should be last visit
sum(K_plotcomplete$nx.plot) ## 93 plants that have some growth at some point. 

## Vector of first encounters, length n; make encounters {1,2,3,4}
encounters <- intra.2013complete %>%
  ...
  ungroup() %>%
  group_by(Site, Plot, Tag) %>%
  summarise(e = min(encounter)) %>%
  arrange(Plot, Site, Tag)

## Should only be ones alive at some point during the survey
length(encounters$e)  ## 93, any plant that had any growth in 2013. 
