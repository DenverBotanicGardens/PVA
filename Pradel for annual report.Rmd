...
## DON'T THINK I need to do this! Detection is ekpt constant in the model, will be no confounding with survival
## Just for 2024
...
lambda.table <- get.real(asmiPradellambdaSouthBeaver$Phi.dot.p.dot.Lambda.dot, "Lambda", se = TRUE)
lambda.table  %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  rename(Lambda.estimates = estimate,
         Lambda.lcl = lcl,
...
asmiPradellambdaannSouthBeaver <- run.Pradlambdaann()

lambda.table <- get.real(asmiPradellambdaannSouthBeaver$Phi.timesite.p.dot.Lambda.timesite, "Lambda", se = TRUE)

SouthBeaverLambdaPlot <- lambda.table %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  mutate(time = as.numeric(as.character(time))) %>%
ggplot(  aes(time, log(estimate), color = group)) +
  geom_point(position = position_dodge(width = 0.2)) +
