...
pRecruitment <- ggplot(f.tableSBCPradrec, aes(ppt_winter, f.estimates, color = as.factor(Site))) +
                    geom_point(position = position_dodge(width = 0.1)) +
                    geom_errorbar(aes(ymin = f.lcl, ymax = f.ucl),position = position_dodge(width = 0.1))+
                    theme_bw() +
                    stat_smooth(method = "gam") +
                    ylab("Per capita recruitment")+
...
## Per captia recruitment and summer maximum temp
ggplot(f.tableSBCPradrec, aes(tmax_springSummer, f.estimates, color = as.factor(Site))) +
                    ...
                    ylab("Per capita recruitment")+
                    scale_color_manual("Site", values = c("darkblue","skyblue","gold","brown")) +
                    xlab("Summer maximum temperatures") +
                    facet_wrap(~ fenced,
                               labeller = labeller(fenced = c("n" = "Open",
                                                   "y" = "Fenced")))
...
Figure 4a and 4b: for the Pradelrec annalyses   
```{r}

ggsave(paste(savepath, "Fig4PradelPerCapRec.jpg", sep = ""),
       
       ccPRecruitment + pRecruitment +
         plot_layout(design = "
                     ABB
                     "),
       width = 275, height = 90, units = "mm", dpi = 300)


```


## Lambda for both Drainages Figure 4a and 4b   
```{r}
...
ggsave(paste(savepath, "Fig3PradelLambda.jpg", sep = ""),
       lambdaCC + lambdaSBC +
         plot_layout(design = "
                     ...
                     #BB
                     "),
width=275, height=180,units='mm', dpi=300)
