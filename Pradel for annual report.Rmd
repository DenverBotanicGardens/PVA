...
run.PradrecSBC <- function(){
  ...
  Phi.siteWinter = list(formula =  ~ WinterPPT_scaled:Site:fence)
  ...
  f.siteWinter = list(formula =  ~ WinterPPT_scaled:Site:fence)
...
f.tableSBCPradrec <- get.real(asmiPradelmodelsSBC$Phi.siteWinter.p.dot.f.siteWinter, "f", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  dplyr::rename(f.estimates = estimate,
         f.lcl = lcl,
         f.ucl = ucl) %>%
...
pRecruitment <- ggplot(f.tableSBCPradrec, aes(WinterPPT_scaled, f.estimates, color = as.factor(Site))) +
...
Phi.tableSBCPradrec <- get.real(asmiPradelmodelsSBC$Phi.siteWinter.p.dot.f.siteWinter, "Phi", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  dplyr::rename(Phi.estimates = estimate,
         Phi.lcl = lcl,
         Phi.ucl = ucl) %>%
