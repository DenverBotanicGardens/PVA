...
```{r}
# Cebolla Creek
f.mpm <- get.real(asmiPradelmodelsCebolla$Phi.siteWinter.p.dot.f.siteWinter, "f", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  group_by(Plot) %>%
...
f.mpmSBCsite <- get.real(asmiPradelmodelsSBC$Phi.siteWinter.p.dot.f.siteWinter, "f", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  mutate(time = as.numeric(as.character(time)),
         group = as.integer(as.character(group))) %>%
  left_join(PlotSiteYrFence, join_by("group" == "plot", "time" == "year")) %>%
...
f.mpmSBC <- get.real(asmiPradelmodelsSBC$Phi.siteWinter.p.dot.f.siteWinter, "f", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  mutate(time = as.numeric(as.character(time)),
         group = as.integer(as.character(group))) %>%
  left_join(PlotSiteYrFence, join_by("group" == "plot", "time" == "year")) %>%
...
##Winter precipitation, summer maximum temp
asmiddlPradlambda.CC$Phi <- asmiddlPradlambda.CC$Phi  %>%
  mutate(Prev12 = as.factor(as.numeric(as.character(time)))) %>%
...
run.asmi.lambda.CC <- function(){
  ...
  Phi.Site = list(formula =  ~ Site)
  Phi.SiteWinter = list(formula =  ~ WinterPPT_scaled:Site)
  ...
  Lambda.SiteWinter = list(formula =  ~ WinterPPT_scaled:Site)
...
Pradlambda.tableCC <- get.real(asmi.lambdaresults.CC$Phi.Site.p.dot.Lambda.SiteWinter, "Lambda", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
...
ggplot(Pradlambda.tableCC, aes(WinterPPT_scaled, logLambda, color = Plot)) +
  geom_point(position = position_dodge(width = .1)) +
...
run.asmi.lambda.SBC <- function(){
  ...
  Phi.SiteWinter = list(formula =  ~ WinterPPT_scaled:Site:fenced)
  ...
  Lambda.SiteWinter = list(formula =  ~ WinterPPT_scaled:Site:fenced)
...
Pradlambda.tableSBC <- get.real(asmi.lambdaresults.SBC$Phi.SiteWinter.p.dot.Lambda.Site, "Lambda", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
  dplyr::rename(Lambda.estimates = estimate,
         Lambda.lcl = lcl,
         Lambda.ucl = ucl) %>%
...
## By site
get.real(asmi.lambdaresults.SBC$Phi.SiteWinter.p.dot.Lambda.Site, "Lambda", se = TRUE) %>%
  distinct(estimate, lcl, ucl, .keep_all = TRUE) %>%
