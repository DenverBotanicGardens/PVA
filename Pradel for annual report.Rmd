...
### Some are site that are named by the plot, the additional additional are fenced but only three sites are fenced
ggplot(f.tableSBCPradrec, aes(WinterPPT, f.estimates, color = as.factor(site))) +
  geom_point(position = position_dodge(width = 1)) +
  geom_errorbar(aes(ymin = f.lcl, ymax = f.ucl),position = position_dodge(width = 1))+
  theme_bw() +
  ylab("Per capita recruitment")+
