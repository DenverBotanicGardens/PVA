...
Fixed effects (site, creek, fence) are constant across individuals; random intercepts have parallel lines for each individual (year or plot in these cases)
```{r}
# 2020 report
...
ggsave(paste(savepath, "Fig4AGGsurv.jpg", sep = ""),
...
asmi.merged %>%
  ...
  ungroup() %>%
  dplyr::mutate(across(ppt_fall:tmin_winter, scale, .names = "{.col}_scaled")) %>%
  dplyr::mutate(tmax_fall_bin = cut(tmax_fall_scaled, breaks = 2)) %>%
                                    # breaks = c(min(tmax_fall_scaled)-0.1, 
                                    #            mean(tmax_fall_scaled),
                                    #            max(tmax_fall_scaled)+0.1),
                                    # labels = FALSE))%>%
ggplot(  aes(ppt_springSummer_scaled, surv, color = site))+
  geom_point(pch="|")+
  # facet_wrap(~site)+
  theme_bw()+
  facet_grid(Creek~tmax_fall_bin)+
  ylab("survival")+
  xlab("ppt")+
  scale_color_discrete("")+
  geom_smooth(method = "glm", method.args = list(family = "binomial"),se = TRUE)
...
asmi.merged %>%
  ...
  ungroup() %>%
  dplyr::mutate(across(ppt_fall:tmin_winter, scale, .names = "{.col}_scaled")) %>%
ggplot(  aes(ppt_winter_scaled, surv, color = site))+
  geom_point(pch="|")+
  theme_bw()+
  facet_wrap(~Creek, scales = "free")+
  ylab("survival")+
...
```{r}
glm1 <- glm(surv ~ ppt_fall + ppt_springSummer + ppt_winter + tmax_fall+tmax_springSummer +
tmax_winter+tmin_fall+tmin_springSummer + tmin_winter, family = poisson(link = 'log'), data = asmi.merged)
glm2 <- glm(surv ~ ppt_fall + ppt_springSummer + ppt_winter + tmax_springSummer +
              tmin_winter , family = poisson(link = 'log'), data = asmi.merged)
glm3 <- glm(surv ~ ppt_winter + tmax_springSummer +
              tmin_winter , family = poisson(link = 'log'), data = asmi.merged) #Need add creek!
glm4 <- glm(surv ~ ppt_springSummer, family = poisson(link = 'log'), data = asmi.merged)
glm5 <- glm(surv ~ 1, family = poisson(link = 'log'), data = asmi.merged)
glm6 <- glm(surv ~ ppt_winter, family = poisson(link = 'log'), data = asmi.merged)
glm7 <- glm(surv ~ tmax_springSummer, family = poisson(link = 'log'), data = asmi.merged) 
glm8 <- glm(surv ~ ppt_fall, family = poisson(link = 'log'), data = asmi.merged)
...
Fig2_a <- ggplot(ps.asmi, aes(Year, Total.Alive, color = as.factor(Site)))+
  stat_smooth(se = FALSE)+
  geom_point()+
  ggtitle("a)")+
  ...
  scale_colour_discrete(guide="none") #, values = terrain.colors(6))

Fig2_b <- ggplot(ps.asmi[ps.asmi$Year > 2013,], aes(Year, Total.Alive, colour = as.factor(Site)))+
  stat_smooth(se = FALSE)+
  geom_point()+
  ggtitle("b)")+
  facet_wrap(~Site, scales = "free_y")+
...
ggplot(  aes(Year, log(Total.Fruit)))+
  # geom_boxplot()+
  geom_point()+
  stat_smooth()+
...
ggsave(paste(savepath, "Fig4.jpg", sep = ""),
