...
levels(asmi.1$status) # 3 is reproductive; 5 is dead
notdeadyet <- do.call(rbind,lapply(split(asmi.1, asmi.1$AsMi_tag_id), function(x){
  data.frame(Tag=unique(x$AsMi_tag_id),
             StageLength=sum(rle(as.numeric(x$status[with(x,order(year))]))$lengths[rle(as.numeric(x$status))$values!=5]))
}))

asmi.raw[asmi.raw$AsMi_tag_id==10,]
...
```{r}
library(ggmap)
ggmap(get_map(c(-107.25,38.25,-106.75,38.75)))+
  geom_point(data=asmiLL, aes(coords.x1,coords.x2))
