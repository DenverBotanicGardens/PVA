...
asmi.c2$Date <- as.Date(paste(asmi.c2$Date, 1, sep="-"), "%Y-%m-%d") 
asmi.c.wide <- melt(asmi.c2, id = c("Site","Plot","Creek","Site.1","Date",
                                    "Year","Month","Prev12","season"))
foo <- dcast(asmi.c.wide, Date+Site+Plot+Creek+Year+Prev12+Month~variable, max )

foo[,4:9] <- lapply(foo[,4:9], function(x) as.numeric(as.character(x)))

foo1 <- melt(foo[,1:9], id.vars = c("Month","Prev12","Site","Plot","Date","Year"),
                 variable.name = "climate", value.name = "climate_val")
foo2 <- dcast(foo1, Plot+Prev12+Site+Month+Date~climate, max)

# now have precip, tmin, and tmax as months over the years. Are the years for Prev12 correct?
foo3.precip <- dcast(foo2, Plot+Prev12+Site~Month, max)
foo3.tmin <- dcast(foo2[,-c(7:8)], Plot+Prev12+Site~Month, max)
foo3.tmax <- dcast(foo2[,-c(6,8)], Plot+Prev12+Site~Month, max)


ps.asmi$X..Flowered.1[is.na(ps.asmi$X..Flowered.1)] <- 0

asmi.c.ps.asmi <- merge(foo, ps.asmi, by.x = c("Plot","Site","Prev12"),
                        by.y = c("Plot","Site","Year"))
...
indx <- c(3,5:9)
asmi.c.ps.asmi[indx] <- lapply(asmi.c.ps.asmi[indx], function(x) as.numeric(as.character(x)))
