...
All
```{r}
lmdSD(asmi.pva$pro.matrix, 1996, currentyr-1)
#         lmda        SD
...
Cebolla all
```{r}
lmdSD(cebolla.pva$pro.matrix, 2014, currentyr-1)
#         lmda       SD
#[1,] 1.138889 1.038878
paste(round(lmdSD(cebolla.pva$pro.matrix, 2014, currentyr-1)[1],2)," (",
      round(lmdSD(cebolla.pva$pro.matrix, 2014, currentyr-1)[2],2), ")", sep ="")
...
sapply(mats.list[-1], function(x){
  paste(round(lmdSD(x, 2014,currentyr-1)[1],2)," (",
        round(lmdSD(x, 2014,currentyr-1)[2],2),")", sep="")
  })

paste(round(lmdSD(cebolla.pva$notfenced.pro.matrix, 2014,currentyr-1)[1],2)," (",
      round(lmdSD(cebolla.pva$notfenced.pro.matrix, 2014,currentyr-1)[2],2),")", sep="")
```
...
#### START HERE#### why are there 'vegetative' to 'seedling' fertilities?!?!
```{r}
...
creekyear <- aggregate(Precip~Creek+Prev12, 
                       data = asmi.c[asmi.c$Year > 1994,], sum)

mean(creekyear$Precip[creekyear$Creek == "Cebolla Creek"])
mean(creekyear$Precip[creekyear$Creek == "South Beaver Creek"])
