...
# Seems no longer supported. Use ggplot stat_ellipse instead (with colour = for designating circles)
...
Save all sites, all data, and set to 20 years allowed dormant    
...
#Cebolla Mid, Site 1
tbl1a[[1]]
write.table(tbl1a[[1]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)

#Cebolla North, Site 2
tbl1a[[2]]
write.table(tbl1a[[2]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)

#South Beaver Creek, Site 5
tbl1a[[3]]
write.table(tbl1a[[3]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)
#South Beaver Creek, Site 15
tbl1a[[4]]
write.table(tbl1a[[4]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)
#South Beaver Creek, Site 19
tbl1a[[5]]
write.table(tbl1a[[5]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)
#South Beaver Creek, Site 26
tbl1a[[6]]
write.table(tbl1a[[6]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)
```
...
#Cebolla Mid, 1
tbl1b[[1]]
write.table(tbl1b[[1]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)

#Cebolla North, 2
tbl1b[[2]]
write.table(tbl1b[[2]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)

#South Beaver Creek, 5
tbl1b[[3]]
write.table(tbl1b[[3]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)

#South Beaver Creek, 15
tbl1b[[4]]
write.table(tbl1b[[4]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)

#South Beaver Creek, 19
tbl1b[[5]]
write.table(tbl1b[[5]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)

#South Beaver Creek, 26
tbl1b[[6]]
write.table(tbl1b[[6]], "clipboard", sep="\t", row.names=FALSE, col.names = FALSE)
```
...
dev.off()
cors <- c("Total.Alive","X..Browsed","X..Flowered","X..Dormant","Total.Fruit")
write.table(cor(ps.cor[,names(ps.cor) %in% 
...
```{r}
#x1 is p.value, x2 is correlation
...
#significant?
df.c[df.c$X1 < 0.0000001 & df.c$X2 > 0.7 & df.c$X2 < 1,]
...
cor.plot(cor(ps.cor[,names(ps.cor) %in% 
                      unique(grep(paste(cors,collapse="|"),
                                  names(ps.cor), value = TRUE))]),numbers = TRUE)
```
...
summary(step(lm(Total.Alive.y ~ (X..Browsed.x + X..Browsed.y +
                                   X..Flowered.x + X..Flowered.y)^2, 
                data = ps.cor)))


summary(lm(Total.Alive.y ~ X..Browsed.x, data = ps.cor))
summary(lm(Total.Alive.y ~ X..Browsed.y, data = ps.cor))
...
t.test(X..Browsed.y~Fence.y, data = ps.cor[ps.cor$Year.x > 2005,]) #not signficantly different
...
#         lmda       SD
#[1,] 1.138889 1.038878
paste(round(lmdSD(cebolla.pva$Site$`1`, 2014, currentyr-1)[1],2)," (",
      round(lmdSD(cebolla.pva$Site$`1`, 2014, currentyr-1)[2],2), ")", sep ="")


paste(round(lmdSD(cebolla.pva$Site$`2`, 2014, currentyr-1)[1],2)," (",
      round(lmdSD(cebolla.pva$Site$`2`, 2014, currentyr-1)[2],2), ")", sep ="")
...
creekyear <- aggregate(Precip~Creek+Prev12, data = asmi.c[asmi.c$Year > 1994,], sum)

#ppt mm*100
mean(creekyear$Precip[creekyear$Creek == "Cebolla"])/100
mean(creekyear$Precip[creekyear$Creek == "South Beaver"])/100
mean(creekyear$Precip)/100
```
...
```{r}
ps.asmi$Creek[ps.asmi$Site > 4] <- "South Beaver"
ps.asmi$Creek[ps.asmi$Site < 3] <- "Cebolla"

#Total.Plants - AGG and dormant
AGGDorm <- summarySE(ps.asmi, measurevar="Total.Plants", groupvars=c("Fence","Year","Site","Creek"))
#Total.Alive - AGG
AGG <- summarySE(ps.asmi, measurevar="Total.Alive", groupvars=c("Fence","Year","Site","Creek"))

#Total.Fruit
Fruit <- summarySE(ps.asmi, measurevar="Total.Fruit", groupvars=c("Fence","Year","Site","Creek"))

#X..Flowered - %flowered
Repro <- summarySE(ps.asmi, measurevar="X..Flowered", groupvars=c("Fence","Year","Site","Creek"))
...
South Beaver Creek  
```{r}
ggplot(ps.asmi[ps.asmi$Total.Plants>0 & ps.asmi$Year > 1995 & 
                 ps.asmi$Year < currentyr & ps.asmi$Creek == "South Beaver",], 
...
Cebolla  
```{r}
ggplot(ps.asmi[ps.asmi$Total.Plants>0 & ps.asmi$Year > 2013 & 
                 ps.asmi$Year < currentyr & ps.asmi$Creek == "Cebolla",], 
...
AGG
```{r}
ggplot(ps.asmi[ps.asmi$Total.Alive>0 & ps.asmi$Creek == "South Beaver",], 
...
AGG
```{r}
ggplot(ps.asmi[ps.asmi$Total.Alive>0 & ps.asmi$Creek == "Cebolla",], 
...
aggregate(Total.Fruit~Site+Year,data=ps.asmi[ps.asmi$Total.Plants>0 & ps.asmi$Creek == "Cebolla",], mean)

aggregate(Total.Fruit~Site+Year,data=ps.asmi[ps.asmi$Total.Plants>0 & ps.asmi$Creek == "Cebolla",], std.error)

ggplot(ps.asmi[ps.asmi$Total.Plants>0 & ps.asmi$Creek == "Cebolla",], 
...
head(yrsalive[[20]])
yrsalive[[20]][yrsalive[[20]]$EndYear == 2016,]
yrsalive[[1]][is.na(yrsalive[[1]]$Yearsalive),]
yrsalive[[3]][is.na(yrsalive[[3]]$AsMi_plot_id),]

#eliminate na or make them zeros? - why would there be NA? oh, 2016 doesn't mean dead, don't know...
...
pcafruit <- aggregate(Total.Fruit ~ Year, 
                      data = ps.asmi[ps.asmi$Creek == "South Beaver"], function(x){
...
pcadorm[pcadorm$Year == 1995 | pcadorm$Year == currentyr,3] <- 2


ps.pc <- data.frame(asmi.pc[asmi.pc$Creek == "South Beaver",], 
                    Fruit = pcafruit$Total.Fruit[pcafruit$Creek == "South Beaver"],
                    Repro = pcarepro[pcarepro$Creek == "South Beaver","X..Flowered"],
                    PerDorm = pcadorm[pcadorm$Creek == "South Beaver",3])


ps.pc.2 <- merge(ps.asmi, asmi.pc, by.x = c("Creek","Year"), by.y = c("Creek","Prev12"))
...
ggplot(tags.seed15, aes(Survey, Ln, group = Site))+
  stat_summary(fun.y = mean, geom="point")+
  stat_summary(fun.data='mean_cl_normal', geom='errorbar', multi=1)+
  facet_grid(~Site)
