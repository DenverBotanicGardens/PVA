...
paste(round(lmdSD(asmi.pva$pro.matrix, 1996, 2014)[1],2)," (",
      round(lmdSD(asmi.pva$pro.matrix, 1996, 2014)[2],2), ")", sep ="")
```
...
```{r}
matord <- c("asmi.pva$Site", "asmi.pva$fenced","asmi.pva$notfenced")
tbl4.96.04 <- 
  lapply(as.character(unique(asmi.2$site)), function(x){
    lapply(matord, function(y){
      paste(round(lmdSD(paste(y,"[",x,"]",sep=""))[1],2)," (",
            round(lmdSD())
    })
  })
