...
jpeg(paste("Q:/Research/All_Projects_by_Species/Astragalus SPECIES/Astragalus_microcymbus/Asmi_Excel/Yearly Summaries/",
         ...
         units = "mm", res = 300, height = 100, width = 200)
...
jpeg(paste("Q:/Research/All_Projects_by_Species/Astragalus SPECIES/Astragalus_microcymbus/Asmi_Excel/Yearly Summaries/",
              currentyr,"_asmi/","Fig2-latticeplot_6Sites.jpg", sep = ""),
         units = "mm", res = 300, height = 100, width = 200)
xyplot(Total.Alive ~ Year|as.factor(Site), ps.asmi.t,
       type = c("g","p","r"),
       layout = c(3, 2),
...
length(dorms$dorms[dorms$dorms > 20]) #13 -> 13
...
asmi.pva <- StagePVA(asmi.all2[asmi.all2$site>2,])
...
```{r}
lm1 <- lm(Lmda ~ ppt.fall + ppt.winter + ppt.springSummer +
            tmax.winter + tmax.fall + tmax.springSummer +
            tmin.springSummer + tmin.fall + tmin.winter, aic.asmi)
lm2 <- lm(Lmda ~ ppt.springSummer, aic.asmi)
lm3 <- lm(Lmda ~ tmin.winter + tmax.springSummer, aic.asmi) 
lm4 <- lm(Lmda ~ ppt.winter, aic.asmi)
lm5 <- lm(Lmda ~ 1, aic.asmi)
lm6 <- lm(Lmda ~ AsMi_site_id, aic.asmi)

# lm7 <- lm(Lmda ~ Prev12, aic.asmi)
lm7 <- lm(Lmda ~ year, aic.asmi)
lm8 <- lm(Lmda ~ ppt.fall + ppt.winter +
            tmax.winter + tmax.fall +  tmax.springSummer +
            tmin.fall + tmin.winter, aic.asmi)
lm9 <- lm(Lmda ~ ppt.winter*tmax.winter*tmin.winter, aic.asmi)
lm10 <- lm(Lmda ~ ppt.fall*tmax.fall*tmin.fall, aic.asmi)
lm11 <- lm(Lmda ~ ppt.fall*ppt.winter*tmax.winter*tmax.fall*tmax.springSummer*tmin.fall*tmin.winter, aic.asmi)
...
```
# if you were to pick one seasonal factor to describe annual growth rate, what would it be?
```{r}
lm1 <- lm(Lmda ~ ppt.fall, aic.asmi)
lm2 <- lm(Lmda ~ ppt.springSummer, aic.asmi)
lm3 <- lm(Lmda ~ ppt.winter, aic.asmi)
lm4 <- lm(Lmda ~ tmin.winter, aic.asmi)
lm5 <- lm(Lmda ~ tmin.springSummer, aic.asmi) 
lm6 <- lm(Lmda ~ tmin.fall, aic.asmi)
lm7 <- lm(Lmda ~ 1, aic.asmi)
lm8 <- lm(Lmda ~ AsMi_site_id, aic.asmi)
lm9 <- lm(Lmda ~ tmax.winter, aic.asmi)
lm10 <- lm(Lmda ~ tmax.fall, aic.asmi)
lm11 <- lm(Lmda ~ tmax.springSummer, aic.asmi)
