...
asmi.2$stage <- ordered(asmi.2$stage, levels = stages[-6])
# check transitions
...
median(dorms) #1
length(dorms[dorms==1]) #692
length(dorms) #1066
length(dorms[dorms > 5]) #16
```
...
# pattern is:
All pre-fence  1996-2004         All post-fence                            Open plots
`asmi.pva$pro.matrix`                 
`asmi.pva$fenced.pro.matrix`     `asmi.pva$fenced.pro.matrix` 2005-2013
`asmi.pva$notfenced.pro.matrix`  `asmi.pva$notfenced.pro.matrix` 2005-2013 `asmi.pva$notfenced` 1996-2013
      
      
Site 5  1996-2004 (which is through 2005 but starting transition of 2004)
`asmi.pva$Site$"5"`  
`asmi.pva$fenced

```{r}
tbl4 <- 
  lapply(unique(asmi.2$site), function(x){
    lapply()
  })
