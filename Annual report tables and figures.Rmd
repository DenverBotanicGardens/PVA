...
## maybe need to cast 1:7 for the current year, 8:12 for the next year and cbind?
asmi.c[asmi.c$Year<1982,]
...
#asmi.cast1.7 <- asmi.cast[!is.na(asmi.cast$`1_Precip`),]
#asmi.cast8.12 <- asmi.cast[!is.na(asmi.cast$`10_Precip`),]

# The first row of asmi.cast1.7 should go away beause I don't have matching 1979 data for the months 8:12. Get rid of first year for each plot of asmi.cast1.7 and last row of asmi.cast8:12 and cbind. 
...
ps.asmi$X..Flowered.1[is.na(ps.asmi$X..Flowered.1)] <- 0

#Merging in two rows for each Plot and Site for months 1:7 and another for months 8:12! Yargg
asmi.c.ps.asmi <- merge(ps.asmi, foo[,-6],  by.y = c("Plot","Site","Prev12"),
                        by.x = c("Plot","Site","Year"))
logtrans <- c("Total.Alive", "X..Browsed",
              "X..Flowered.1","X..Flowered","X..Dormant","Total.Fruit")

log.t.cols <- do.call(data.frame, lapply(asmi.c.ps.asmi[,logtrans], function(x){
  log(x + 1)
}))
names(log.t.cols) <- sapply(logtrans, function(x) paste(x, "log", sep=""))
asmi.c.ps.asmi <- data.frame(asmi.c.ps.asmi, log.t.cols)

asmi.c.ps.asmi <- subset(merge(asmi.c.ps.asmi, asmi.c.ps.asmi,
                               by = c("Plot","Site")),
                         Prev12.x == Prev12.y -1)
...
lm1 <- lm(Total.Alivelog.y ~ Total.Alivelog.y+X..Browsedlog.y+X..Floweredlog.y+
          ...
          data=asmi.c.ps.asmi)
#only fruit last year
lm2 <- lm(Total.Alivelog.y ~ Total.Fruitlogtransformed.x, data=asmi.c.ps.asmi)
# only browsed last year
lm3 <- lm(Total.Alivelog.y ~ X..Browsedarcsinsqrttransformed.x, data=asmi.c.ps.asmi)
# nothing
lm4 <- lm(Total.Alivelog.y ~ 1, data=asmi.c.ps.asmi)
# lm1 also alive last year
...
lm6 <- lm(Total.Alivelog.y ~ Total.Alivelogtransformed.x+
            X..Browsedarcsinsqrttransformed.x +
            Total.Fruitlogtransformed.x + X..Flowered.y +
            X..Browsedarcsinsqrttransformed.y, data=asmi.c.ps.asmi)
# and alive given browsed last year
lm7 <- lm(Total.Alivelog.y ~ Total.Alivelogtransformed.x+ #total AGG last year
            ...
            X..Browsedarcsinsqrttransformed.y, data=asmi.c.ps.asmi)
# only fenced
lm8 <- lm(Total.Alivelog.y ~ Fence.x, data=asmi.c.ps.asmi)
# browsed last year, fruit this year
