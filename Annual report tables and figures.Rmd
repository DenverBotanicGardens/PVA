...
asmicoty <- unique(dm$UniqueID[grep(paste(toMatch, collapse="|"),
                                          dm$Comment)])
# Need only surveys after first seedling time that aren't seen marked as dormant
dm[dm$UniqueID %in% asmicoty,]

# seedlingones <- lapply(asmicoty, function(x){
#   dm[dm$UniqueID == x,]
# })

# seedlingones[[2]] # cotyledon that was either subsummed or died Comment %in% "subsumed" <- dormant
# seedlingones[[100]] # after may 2015 should be dormant for a couple times, then maybe dead
...
dm$status[grep("seedling", dm$Comment)] <- "seedling"
dm$status[grep("coty",dm$Comment)] <- "seedling"
dm$status[grep("Coty",dm$Comment)] <- "seedling"

table(dm$Comment[dm$UniqueID %in% asmicoty])
# What else are dormant?
...
table(dm$Year, dm$status)
table(dm$Comment[dm$Ln<5 & dm$UniqueID %in% notseedlingones])
