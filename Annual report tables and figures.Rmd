...
ggplot(ps.asmi.t, aes(X..Dormant))+
  geom_histogram()+
  facet_grid(~Site)

ggplot(ps.asmi.t, aes(Percent..Dormant))+
  geom_histogram()+
  facet_grid(~Site)

logtrans <- c("Total.Alive", #Percent Browsed
...
ggplot(ps.asmi.t, aes(Percent..Dormantlogtransformed))+
  geom_histogram()+
  facet_grid(~Site)
...
```{r}
assqrt <- do.call(data.frame,lapply(ps.asmi.t["X..Browsed"],
...
```{r}
cor.vars <- length(names(ps.cor[,grep("transformed",names(ps.cor))]))

cortests <- apply(combn(1:cor.vars,2), 2, function(x){
  cor.df <- ps.cor[,grep("transformed",names(ps.cor))]
...
```{r}
cor.out <- round(cor(ps.cor[,grep("transformed",names(ps.cor))]),2)
...
#x = t, y = t+1
# X..Flowered = density; X..Flowered.1 = % reproductive  
...
#All interactions
step1 <- as.formula(paste("Total.Alivelogtransformed.y ~ (",
                          paste(names(ps.cor)[grep("transformed",names(ps.cor))[-7]], #remove Total.Alivelogtransformed.y from the variables!
                                collapse = "+"),")^2"))
lm4step <- do.call("lm", list(step1, quote(ps.cor)))
summary(step(lm4step))

#Just linear factors
step2 <- as.formula(paste("Total.Alivelogtransformed.y ~ ",
                          paste(names(ps.cor)[grep("transformed",names(ps.cor))[-7]],
                                collapse = "+")))
lm4step2 <- do.call("lm", list(step2, quote(ps.cor)))
summary(step(lm4step2))

ggplot(ps.cor, aes(x=Total.Fruitlogtransformed.y,y=Total.Alivelogtransformed.y) )+
  geom_point()+
  stat_smooth(method = "lm")

ggplot(ps.cor, aes(x=Total.Alivelogtransformed.y,y=Total.Fruitlogtransformed.y) )+
  geom_point()+
  stat_smooth(method = "lm")
