...
step1 <- as.formula(paste("Total.Alivelogtransformed.y ~ (",
                          paste(names(ps.cor)[grep("transformed",names(ps.cor))[-out]], #remove Total.Alivelogtransformed.y from the variables!
                                collapse = "+"),")^2"))
...
beforeandafterstage <- do.call(rbind,apply(seqXclass, 1, function(x){
  dormant <- which(x == x[(x=="0")])
  dormant <- dormant[dormant>1&dormant<length(x)]
  if(length(dormant)>0){
    outbefore <- x[dormant-1]
    outafter <- x[dormant+1]
    if(length(outbefore[outbefore!="0"])==0){
      outbefore <- NA
    } else {
      outbefore <- outbefore[outbefore!="0"]
    }
    
    if(length(outafter[outafter!="0"])==0){
      outafter<-NA
    } else {
      outafter <- outafter[outafter!="0"]
    }
    
    cbind(outbefore,outafter)
    }
  }))

beforeandafterstage<-data.frame(survey=dimnames(beforeandafterstage)[[1]],beforeandafterstage)
str(beforeandafterstage)

##Factor w/ 4 levels "0","D","R","V":
#percent of time goes from Vegetative to dormant
nrow(beforeandafterstage[beforeandafterstage$outbefore=="V",])/
