...
dm$status[is.na(dm$Ln) & !is.na(dm$Comment)]
table(dm$Comment[is.na(dm$Ln) & !is.na(dm$Comment)])
...
toMatch <- c("coty","Coty","toothpi")
dm$status[grep(paste(toMatch, collapse="|"),
               dm$Comment, ignore.case=TRUE)] <- "seedling"

# dm$status[dm$Comment %in% c("coty","Coty","toothpi")] <- "seedling"
...
unique(intra.2013July$Tag)
unique(dm$tag[dm$UniqueID %in% notseedlingones])
for(i in notseedlingones){
  # firstseen <- min(dm$Date[dm$UniqueID==i & dm$Ln > 0])
  
  # Assign first time as seedling if really small ?? no, don't really know 
  # if(dm$Ln[dm$Date == firstseen] < 5){
  #   
  # }
...
ggplot(dm.climate, aes(as.factor(Year), Count, colour = status))+
  geom_jitter()+
  geom_boxplot()+
  theme_bw()

# What is detected in July in a bad to good year
ggplot(dm.climate[dm.climate$Month == "07",], aes(as.numeric(Year), Count, colour = status))+
  geom_point()+
  # geom_boxplot()+
  theme_bw()+
...
ggplot(dm.climate, aes(as.numeric(Year), Count, colour = status))+
  geom_point()+
  # geom_boxplot()+
  theme_bw()+
  stat_smooth(method="lm")
...
# proportions
ggplot(dm.season[dm.season$Month == "07",], aes(as.numeric(Year),  fill = status))+
  geom_bar()+
  # geom_boxplot()+
  theme_bw()
```
...
```{r}
table(dm.season$status[dm.season$Month=="07"],dm.season$Year[dm.season$Month=="07"])[2,1] # Dormant in 2014
colSums(table(dm.season$status[dm.season$Month=="07"],dm.season$Year[dm.season$Month=="07"])) # 1909 both years
