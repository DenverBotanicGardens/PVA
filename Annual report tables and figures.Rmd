...
asmi.merged <- asmi.raw %>%
  ...
  dplyr::select(AsMi_tag_id:status.x, AsMi_plot_id:fence.x, year.y:status.y) %>%
...
asmi.merged %>%
ggplot( aes(year.y, surv, color = fence.x))+
  geom_smooth(method = "glm", method.args = list(family = "binomial"),se = TRUE)+
  geom_point(shape = "|")+
  theme_light()+
  xlab("Year")+
...
glm4 <- glm(surv ~ year.x*fence.x, family = binomial, data = asmi.merged)
...
glm8 <- glm(surv ~ fence.x, family = binomial, data = asmi.merged)
