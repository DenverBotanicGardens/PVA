...
head(ps.cor[,c("Year.x","Year.y")]) # x = t, y=t+1
```
...
ps.cor has both current (.x) = t      
and previous year (.y) = t+1
```{r}
cor.vars <- length(names(ps.cor[,names(ps.cor) %in% 
                      unique(grep(paste(cors,collapse="|"),
                                  names(ps.cor), value=TRUE))]))

cortests <- apply(combn(1:cor.vars,2), 2, function(x){
  cor.df <- ps.cor[,names(ps.cor) %in% unique(grep(paste(cors,collapse="|"),
                                                   names(ps.cor), value=TRUE))]
...
```{r}
cor.out <- round(cor(ps.cor[,names(ps.cor) %in% 
...
cor.sign <- apply(notsig,1,function(x){
  row1 <- which(dimnames(cor.out)[[1]] == x[1]) #row
  column2 <- which(dimnames(cor.out)[[2]] == x[2]) #column
  for(i in 1:length(dimnames(cor.out)[[1]])){
    for(j in 1:length(dimnames(cor.out)[[2]])){
      if(i == row1){
        if(j == column2){
          cor.out[i,j] <- paste(cor.out[i,column2],"*",sep="")
          } else {
            cor.out[i,j] <- cor.out[i,j]
          }
        }
      }
  }
...
Backwards stepwise multiple linear regressions   
Herbivory   
Browsing is already arcsine aquare root transformed
y is the current year
x is the previous year    
Total.Alive is AGG 
```{r stepwise multiple linear regressions}

step1 <- as.formula(paste("Total.Alive.y ~ (",
                          paste(names(ps.cor)[grep(paste(cors[-1],collapse="|"),
                                                   names(ps.cor))],
                                collapse = "+"),")^2"))
lm4step <- do.call("lm", list(step1, quote(ps.cor)))
summary(step(lm4step))

step2 <- as.formula(paste("Total.Alive.y ~ ",
                          paste(names(ps.cor)[grep(paste(cors[-1],collapse="|"),
                                                   names(ps.cor))],
                                collapse = "+")))
lm4step2 <- do.call("lm", list(step2, quote(ps.cor)))
summary(step(lm4step2))


summary(step(lm(Total.Alive.y ~ (X..Browsed.x + X..Flowered.1.x + X..Browsed.y +
             X..Flowered.1.y + X..Dormant.y)^2, data= ps.cor)))

summary(lm(Total.Alive.y ~ X..Browsed.x, data = ps.cor)) #previous year
summary(lm(Total.Alive.y ~ X..Browsed.y, data = ps.cor)) #current year

summary(lm(Total.Alive.y ~ X..Flowered.x, data = ps.cor))
summary(lm(Total.Alive.y ~ X..Flowered.y, data = ps.cor))

sort(unique(ps.cor$Year.x))
sort(unique(ps.cor$Year.y))

t.test(X..Browsed~Fence, data = ps.asmi.t)

#In all years after fence installation
t.test(Total.Fruit.y~Fence.y, data = ps.cor[ps.cor$Year.x > 2005,])
...
Nooooo, why lengths of 40?! can't be. Unless some individuals incorretly always say dormant or end or start with dormant? removed 1995 so can start in 1996 with dormant
...
table(dorms) #No!! study hasn't been long enough, cannot be any that are ending in dormant!

table(asmi.2$year) #Oh! missing 2017 because that's fate, not stage!

which(rle(as.numeric(asmi.2$stage))$lengths > (currentyr-1995)) #first one is at 67  
# all the places in asmi.2 where it goes from dormant in one tag to dormant in another
#   67  833 1317 2542 2562 2586 2647 2887 2893 3081 3959
sum(rle(as.numeric(asmi.2$stage))$lengths[1:67]) #206

fromtodorms <- lapply(which(rle(as.numeric(asmi.2$stage))$lengths > (currentyr-1995)),
                      function(x){
                        rows <- sum(rle(as.numeric(asmi.2$stage))$lengths[1:x])
                        out <- asmi.2[(rows-10):(rows+5),]
                        out
                      })

fromtodorms[[1]]

laststage <- do.call(rbind,lapply(unique(asmi.2$tag), function(tag){
  currenttag <- asmi.2[asmi.2$tag == tag,] 
  out <- currenttag[currenttag$year == max(currenttag$year),]
  out
})
)

nrow(laststage[laststage$fate == "dormant",]) # Well goodness! First off, tag 1 has 2017!
laststage[laststage$fate == "dormant",]
...
```{r}
median(dorms) #3
length(dorms[dorms==1]) #305
length(dorms[dorms==1])/length(dorms) # 37.7%
length(dorms) #808
length(dorms[dorms > 5]) #286
length(dorms[dorms > 15]) #50
length(dorms[dorms == 20]) #20
```
...
ggplot(clim_agg_long[clim_agg_long$variable%in% c("Precipitation (mm)","Individuals per plot"),], 
       aes(Year, value))+
#  geom_point()+
...
names(comp1[1])
MidL <- unique(ave(season[,names(comp1[1])],season$Creek)) -
  aggregate(season[,names(comp1[1])],by = list(season$Creek), sd)$x
MidH <- unique(ave(season[,names(comp1[1])],season$Creek)) +
  aggregate(season[,names(comp1[1])],by = list(season$Creek), sd)$x
Mid <- unique(ave(season[,names(comp1[1])],season$Creek))
```
...
AGGDorm <- summarySE(ps.asmi.t, measurevar="Total.Plants", groupvars=c("Fence","Year","Site","Creek"))
#Total.Alive - AGG
AGG <- summarySE(ps.asmi.t, measurevar="Total.Alive", groupvars=c("Fence","Year","Site","Creek"))

#Total.Fruit
Fruit <- summarySE(ps.asmi.t, measurevar="Total.Fruit", groupvars=c("Fence","Year","Site","Creek"))

#X..Flowered - %flowered
Repro <- summarySE(ps.asmi.t, measurevar="X..Flowered", groupvars=c("Fence","Year","Site","Creek"))
...
```{r}
ggplot(ps.asmi[ps.asmi.t$Total.Alive>0 & ps.asmi.t$Creek == "South Beaver",], 
  ...
  scale_color_manual("Site",
                     values=c("#000000","#E0E0E0","#00FF00","#FF9933"),
                     labels = c("Site 5","Site 15","Site 19","Site 26"))+
  scale_shape_manual("Site",
                     values=c(5,15,19,26),
                     labels = c("Site 5","Site 15","Site 19","Site 26"))
...
AGG
```{r}
ggplot(ps.asmi.t[ps.asmi.t$Total.Alive>0 & ps.asmi.t$Creek == "Cebolla",], 
  ...
  theme(legend.justification=c(0,1), legend.position=c(0,1)) #upper left
...
South Beaver Creek
```{r}
ggplot(ps.asmi[ps.asmi$Total.Plants>0 & ps.asmi$Creek == "South Beaver",], 
...
aggregate(Total.Fruit~Site+Year,data=ps.asmi.t[ps.asmi.t$Total.Plants>0 & ps.asmi.t$Creek == "Cebolla",], mean)

aggregate(Total.Fruit~Site+Year,data=ps.asmi.t[ps.asmi.t$Total.Plants>0 & ps.asmi.t$Creek == "Cebolla",], std.error)

ggplot(ps.asmi.t[ps.asmi.t$Total.Plants>0 & ps.asmi.t$Creek == "Cebolla",], 
...
ggplot(ps.asmi.t[ps.asmi.t$Total.Plants>0 & ps.asmi.t$Creek == "Cebolla",], 
  ...
  scale_shape_manual("Site",
                     ...
                     labels = c("Cebolla Mid","Cebolla North"))+
  scale_x_continuous(breaks = c(2014:currentyr))+
  theme(legend.justification=c(0,1), legend.position=c(0,1)) #upper right
```
...
Mode(dorms)
length(dorms[dorms==1]) #322
length(dorms[dorms==1])/length(dorms)
length(dorms[dorms == max(dorms)]) # 14 with 20 years
...
t.test(Comp.2~PerDorm, data = ps.pc[ps.pc$Prev12 > 1995 & ps.pc$Prev12 < 2015,])
...
dm[,names(Filter(is.character, dm))] <- sapply(dm[,names(Filter(is.character, dm))], 
