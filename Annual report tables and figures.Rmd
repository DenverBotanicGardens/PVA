...
xyplot(Total.Alive.x ~ Year.x|as.factor(Site), ps.cor[ps.cor$Year.x > 2013,],
       type = c("g","p","r"),
       index = function(x,y) coef(lm(y~x))[1],
       xlab = "Year", 
       ylab = "Individuals with AGG", aspect = "xy")
...
xyplot(Total.Alive.x ~ Year.x|Fence.x, 
       ps.cor[ps.cor$Creek.x == "SouthBeaver" & ps.cor$Year.x > 2005,],
       type = c("g","p","r"),
       index = function(x,y) coef(lm(y~x))[1],
       xlab = "Year", 
       ylab = "Individuals with AGG", aspect = "xy")
...
xyplot(Total.Alive.x ~ Year.x|Creek.x, ps.cor[ps.cor$Year.x > 2013,],
       type = c("g","p","r"),
       index = function(x,y) coef(lm(y~x))[1],
       xlab = "Year", 
       ylab = "Individuals with AGG", aspect = "xy")
...
```{r}
median(dorms) #3
length(dorms[dorms==1]) #347 -> 358
length(dorms[dorms==1])/length(dorms) # 37%
length(dorms) #848 -> 959
length(dorms[dorms > 5]) #353 -> 355
length(dorms[dorms > 15]) #93 -> 82??
length(dorms[dorms == 21]) #13 -> 0, oh, I downloaded with dorm = 20 I think!
```
...
for(i in unique(surv.asmi$year)){
  logi.hist.plot(surv.asmi$length[surv.asmi$year==i],
                 surv.asmi$surv[surv.asmi$year==i], 
...
for(i in unique(surv.asmi$year)){
  plot(surv.asmi$length[surv.asmi$year==i], 
       surv.asmi$surv[surv.asmi$year==i], xlab="length",ylab="Probability of survival")
...
ggplot(asmi.raw[dm$TagperPlot %in% asmi.raw$TagperPlot,], aes()
```
