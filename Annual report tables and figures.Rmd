...
asmi.merged %>%
  mutate(Creek = case_when(AsMi_site_id < 3 ~ "Cebolla",
                           AsMi_site_id > 3 ~ "South Beaver")) %>%
ggplot( aes(year.y, surv, color = status.x))+
  geom_smooth(method = "glm", method.args = list(family = "binomial"),se = TRUE)+
  geom_point(shape = "|")+
  theme_light()+
  xlab("Year")+
...
glm1 <- glm(flower ~ ppt_winter, family = binomial, data = mastseeding)
glm2 <- glm(flower ~ tmax_springSummer*ppt_winter, family = binomial, data = mastseeding)
glm3 <- glm(flower ~ tmin_winter*ppt_winter, family = binomial, data = mastseeding) 
glm4 <- glm(flower ~ ppt_fall, family = binomial, data = mastseeding)
glm5 <- glm(flower ~ 1, family = binomial, data = mastseeding)
glm6 <- glm(flower ~ ppt_springSummer, family = binomial, data = mastseeding)
glm7 <- glm(flower ~ tmax_springSummer, family = binomial, data = mastseeding) 
glm8 <- glm(flower ~ tmin_winter, family = binomial, data = mastseeding)
glm.list <- list(glm1,glm2,glm3,glm4,glm5,glm6,glm7,glm8)
glm.names <- as.character(unlist(lapply(glm.list,formula)))
(glm.results <- aictab(glm.list, modnames=glm.names))
for(i in 2:length(glm.list)){
...
ggsave(paste(savepath, "Fig4mastnofruit.jpg", sep = ""),
       ## d) fruit or no fruit
noreproductiveyears %>%
  mutate(site = case_when(AsMi_site_id == 1 ~ "Cebolla Mid",
                          AsMi_site_id == 2 ~ "Cebolla North",
  ...
  ungroup() %>% 
  dplyr::mutate(across(ppt_fall:tmin_winter, .fns = scale)) %>%
ggplot(  aes(ppt_springSummer,flower))+ #,colour=as.factor(AsMi_site_id)))+
  geom_point()+
  theme_bw()+
