...
asmi.c %>%
  group_by(Prev12) %>%
  summarise(meanPPT = mean(totalPPT), sdPPT = sd(totalPPT),
            meanTmax = mean(avgMontmax), sdTmax = sd(avgMontmax),
            meanTmin = mean(avgMontmin), sdTmin = sd(avgMontmin))
...
#   left_join(asmi.raw, by = c("AsMi_tag_id", "AsMi_plot_id", "AsMi_site_id")) %>%  ## not working "fence", "year.x" == ("year.y" + 1)
...
## https://stackoverflow.com/questions/75384121/add-multiple-columns-lagged-by-one-year 
asmi.merged <- asmi.raw %>%
  group_by(year,AsMi_site_id,AsMi_plot_id,fence) %>%
  mutate(length.y = lag(length),
         flower.y = lag(flower),
         fruit.y = lag(fruit),
         browsing.y = lag(browsing),
         fate = lag(status),
         year.y = lag(year),
         year = as.integer(as.character(year)) - 1) %>%
  ungroup() %>% #  .y is year t-1
  dplyr::select(AsMi_tag_id:status, AsMi_plot_id:fence, age:year.y) %>%
  filter(length > 0) %>%
  filter(!is.na(length)) %>%
  left_join(asmi.season.wide,  by = c("AsMi_plot_id" = "Plot",
                                      "year.y" = "Prev12")) %>%
...
asmi.merged %>%
  dplyr::select(c(year:length,status,fate,age,surv,year.y))
...
ggplot(  aes(tmax_springSummer,surv,color = site))+
  ...
  facet_grid(~ppt_winter_bin, scales = "free", 
             labeller = labeller(ppt_winter_bin =
                                   c("(-1.87,-0.408]" = "low winter precipitation",
                                     "(-0.408,1.05]"  = "mid winter precipitation",
                                     "(1.05,2.51]"  = "high winter precipitation"))
             )+
...
ggplot(  aes(tmax_springSummer,FlYN))+
  ...
  facet_grid(~ppt_winter_bin, scales = "free", 
              labeller = labeller(ppt_winter_bin =
                                    c("(-1.87,-0.317]" = "low winter precipitation",
                                      "(-0.317,1.23]"  = "mid winter precipitation",
                                      "(1.23,2.77]"  = "high winter precipitation"))
              )+
  xlab("Summer maximum temperature")+
...
ggplot(  aes(tmax_springSummer_bin,log(Total.Fruit), color = site))+
  ...
  facet_grid(~ppt_winter_bin, scales = "free", 
               labeller = labeller(ppt_winter_bin =
                                     c("(-1.26,0.0843]" = "low winter precipitation",
                                       "(0.0843,1.43]"  = "mid winter precipitation",
                                       "(1.43,2.77]"  = "high winter precipitation")))+
...
## Multistate CJS models   
open population capture-recapture model  
Cannot have dormant stage, just vegetative and reproductive
```{r}
library(marked)

## Need capture history (ch) column and a stage column (stage)
...
ggplot(  aes(tmax_springSummer,flower, colour=site))+
  ...
  facet_grid(~ppt_winter_bin, scales = "free",
             labeller = labeller(ppt_winter_bin =
                                   c("(-1.85,-0.226]" = "low winter precipitation",
                                     "(-0.226,1.39]" = "mid winter precipitation",
                                     "(1.39,3.02]" = "high winter precipitation"))
             )+
