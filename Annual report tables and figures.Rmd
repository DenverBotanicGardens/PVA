...
```
# Beta regression or binomial glm with logit link  
```{r}
library(betareg) # for a beta regression, values in (0,1) and  not containing 0 or 1
# need to transform by (y * (n-1) + 0.5)/n where n is the smaple size (see Smithson and Verkuillen 2006)


hist(ps.asmi.reduced$X..Flowered)
hist(ps.asmi.reduced$X..Flowered[!is.na(ps.asmi.reduced$X..Flowered)])

# transform 
ps.asmi.reduced$PercFlTrans <- (ps.asmi.reduced$X..Flowered * ())
```
...
#  But this could be multiple times per individual
levels(asmi.all2$stage)
# test
# dorms.1 <- rle(as.numeric(asmi.all2$stage))$lengths[rle(as.numeric(asmi.all2$stage))$values ==4]
x <- split(asmi.all2, asmi.all2$tag)[[1]]
dorms.1 <- unlist(lapply(split(asmi.all2, asmi.all2$tag), function(x){
  # dormyrs <- x$year[x$stage == "dormant"]
  # dorm <- rle(as.numeric(x$stage))$lengths[rle(as.numeric(x$stage))$values == 4]
  # out <- data.frame(tag = unique(x$tag), DormPeriods = length(dorm), )
