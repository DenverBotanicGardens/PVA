...
ggplot(asmimerg, aes(as.factor(Site),X..Flowered))+
  geom_boxplot()+
  geom_rug()+
  theme_bw()
...
exclude <- c("Fence","Plot","Site","Year","Creek","SiteName",
             "Mar","Apr","May","Jun","Jul","Aug","Sep","logtrans","Plants",
...
variables <- lapply(1:length(x), function(pick){
  combn(x, pick)
})
...
as.formula(paste("y~", paste(names(asmimerg)[25:28], collapse="+")))


(X..Flowered, data=asmimerg)
...
Becky's comment: "For the july censusâ€¦can we say that all seedlings were less than 10 cm and all re-emerging plants are greater than 10cm? I.e., can we use these data to have a cut off for the July census..kind of like we do with Scgl for reproductive vs non-reproductive? If soâ€¦then we could use this to back track and re-assess all prior years data and also use it going forward with just the July census."
