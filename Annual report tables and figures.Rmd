...
#         lmda       SD
#[1,] 1.138889 1.038878
paste(round(lmdSD(cebolla.pva$pro.matrix, 2014, currentyr-2)[1],2)," (",
      round(lmdSD(cebolla.pva$pro.matrix, 2014, currentyr-2)[2],2), ")", sep ="")
...
sapply(mats.list, function(x){
   paste(round(lmdSD(x, 2014, currentyr-1)[1],2)," (",
         round(lmdSD(x, 2014, currentyr-1)[2],2),")", sep="")
    })
```
...
tbl4.96.04 <- 
  lapply(as.character(unique(asmi.2$site)), function(x){
    mat <- sprintf('matord.list[[1]]$\"',x,'\"$\"2004"')
...
SpatialPoints(c(asmilatlong$Northing,asmilatlong$Easting), 
              proj4string=CRS("+proj=utm +zone=13 +datum=WGS84"))

# Specify column names for (x,y) and convert to a SpatialPointsDataFrame
coordinates(asmilatlong) <- c("Easting","Northing")

# Specify the projection
proj4string(asmilatlong) <- CRS("+proj=utm +zone=13, ellps=WGS84")

as(asmilatlong, "SpatialPoints")

asmilatlong <- spTransform(asmilatlong, CRS("+proj=longlat +datum=WGS84"))

asmiLL <- data.frame(asmilatlong@data, long = coordinates(asmilatlong)[,1],
           lat=coordinates(asmilatlong)[,2])
```
...
avgTemps.max <- lapply(maxs, function(x){
  ...
  data.frame(data = extract(rastertemps, asmiLL[,c("long","lat")]), date = ls_prism_data()[x,])
})
...
mins.avg <- cbind(mins.avg, asmilatlong$Tag__)
maxs.avg <- cbind(maxs.avg, asmilatlong$Tag__)
ppt.avg <- cbind(ppt.avg, asmilatlong$Tag__)

 gsub('.*-([0-9]+).*','\\2',asmi.climate$date[1])

asmi.climate <- cbind(mins.avg, maxs.avg[,1], ppt.avg[,1])
names(asmi.climate) <- c("tmin","date","Plot","tmax","Precip")
...
names(asmi.c) <- c("Plot","Site","Creek",names(asmi.c)[4:8])
...
ggplot(asmi.pc, aes(Comp.1,Comp.2, color = Creek))+
  geom_point()+
  stat_ellipse()+
  theme_bw()+
  xlab("PC1 (34%)")+
