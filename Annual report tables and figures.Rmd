...
When PRISM fails (problems in 2025)   
Manual download from Oregon PRISM
```{r}
...
cebolla_prism <- read.csv(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Astragalus-microcymbus/Astragalus-microcymbus_Data/PRISM_ppt_tmin_tmean_tmax_tdmean_vpdmin_vpdmax_stable_4km_189501_202412_38.3603_-107.1467_Cebolla.csv", collapse = "", sep = ""), skip = 10)

cebolla_prism <- cebolla_prism %>%
  mutate(Year = substr(Date, 1,4)) %>%
  mutate(Month = substr(Date, 6,7)) %>%
  dplyr::rename(ppt = ppt..mm.) %>%
  dplyr::rename(tmin = tmin..degrees.C.) %>%
  dplyr::rename(tmax = tmax..degrees.C.) %>%
  dplyr::rename(tmean = tmean..degrees.C.) %>%
  dplyr::rename(vpdmin = vpdmin..hPa.) %>%
  dplyr::rename(vpdmax = vpdmax..hPa.) %>%
  dplyr::rename(tdmean = tdmean..degrees.C.) %>%
  mutate(Month = as.numeric(Month)) %>%
  mutate(Year = as.numeric(Year))
  
cebollaSitePlot <- asmiLL %>%
  dplyr::rename(Site = asmilatlong.Tag_Commen) %>%
  dplyr::rename(Plot = asmilatlong.Tag__) %>%
  dplyr::select(Plot,Site) %>%
  filter(grepl("Cebolla", Site))

cebolla_prism <- cebolla_prism %>%
  crossing(cebollaSitePlot) %>%
  dplyr::mutate(Creek = "Cebolla")
  
## Do one for each site
Site05 <-  read.csv(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Astragalus-microcymbus/Astragalus-microcymbus_Data/PRISM_stable_4km_189501_202412_38.4847_-106.9792_Site5.csv", collapse = "", sep = ""), skip = 10)
...
Site5SitePlot <- asmiLL %>%
  dplyr::rename(Site = asmilatlong.Tag_Commen) %>%
  dplyr::rename(Plot = asmilatlong.Tag__) %>%
  dplyr::select(Plot,Site) %>%
  filter(grepl("Site 5", Site))

Site05 <- Site05 %>%
  crossing(Site5SitePlot) %>%
  mutate(Creek = "South Beaver")

Site15 <-  read.csv(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Astragalus-microcymbus/Astragalus-microcymbus_Data/PRISM_stable_4km_189501_202408_38.4317_-106.9563.csv", collapse = "", sep = ""), skip = 10)
...
Site15SitePlot <- asmiLL %>%
  dplyr::rename(Site = asmilatlong.Tag_Commen) %>%
  dplyr::rename(Plot = asmilatlong.Tag__) %>%
  dplyr::select(Plot,Site) %>%
  filter(grepl("Site 15", Site))

Site15 <- Site15 %>%
  crossing(Site15SitePlot) %>%
  mutate(Creek = "South Beaver")


Site1926 <-  read.csv(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Astragalus-microcymbus/Astragalus-microcymbus_Data/PRISM_stable_4km_189501_202412_38.4409_-106.9571_Site19.csv", collapse = "", sep = ""), skip = 10)
...
Site1926SitePlot <- asmiLL %>%
  dplyr::rename(Site = asmilatlong.Tag_Commen) %>%
  dplyr::rename(Plot = asmilatlong.Tag__) %>%
  dplyr::select(Plot,Site) %>%
  filter((Site %in% c("Site 19","Site 26")))

Site1926 <- Site1926 %>%
  crossing(Site1926SitePlot) %>%
  mutate(Creek = "South Beaver")


asmi.climate <- cebolla_prism %>%
  rbind(Site05) %>%
  rbind(Site15) %>%
  rbind(Site1926)

## Got August 2025 which is one month that goes to fall Prev12 == 2026
asmi.season.2 <- asmi.climate %>%
  ...
  mutate(Prev12 = ifelse(Month > 7, Year+1, Year)) %>%
  group_by(season, Plot, Site, Creek, Prev12) %>%
  ...
  ungroup() %>%
  group_by(season) %>%
  dplyr::mutate(across(ppt:tmax, .fns = scale)) %>%
  pivot_longer(ppt:tmax, names_to = "Variable", values_to = "Value") %>%
  mutate(Value = c(Value))

asmi.season.wide <- asmi.season.2 %>%
  pivot_wider(names_from = c(Variable,season), values_from = Value) %>%
  dplyr::rename(site = Site)

asmi.annual.2 <- asmi.climate %>%
  # Make previous 12 months match year (previous to May survey)
  mutate(Prev12 = ifelse(Month > 7, Year+1, Year)) %>%
  group_by(Plot, Site, Creek, Prev12) %>%
  dplyr::summarise(ppt = sum(ppt),
            tmin = mean(tmin),
            # tmean = mean(tmean),
            tmax = mean(tmax)) %>%
...
save(asmi.season.wide, file = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Astragalus-microcymbus/Astragalus-microcymbus_Data/asmi.season.wide2", currentyr,".Rdata", sep=""))
```
...
asmi.season.wide %>%
  ...
  pivot_longer(cols = ppt_fall:tmin_winter, names_to = "VarSeason",
               values_to = "Value") %>%
  left_join(plots.site, by = c('Plot' = 'AsMi_plot_id', "Creek" = "Creek")) %>%
  separate(VarSeason, c("Variable","season"), sep = "_") %>%
  ggplot(  aes(Prev12, Value, color = site))+
    geom_point()+
    stat_smooth()+
...
```{r}
asmi.season.2 %>%
  distinct(Site, Prev12, Value, Variable, season, Creek) %>%
  ...
  ggplot(   aes(Prev12, Value, color = Site))+
    ...
    theme_bw()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
    scale_color_manual("Site", values = c("black","grey30","deepskyblue","cyan4","cyan","cornflowerblue"))+
    facet_grid(Variable ~ season + Creek, scales = "free_y")


asmi.annual.2 %>%
  distinct(Site, Prev12, Value, Variable, Creek) %>%
  filter(Prev12 < (currentyr + 1)) %>%
  filter(Prev12 > 1994) %>%
  ggplot(   aes(Prev12, Value, color = Site))+
    geom_point()+
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
    scale_color_manual("Site", values = c("black","grey30","deepskyblue","cyan4","cyan","cornflowerblue"))+
    facet_grid(Variable ~ Creek, scales = "free_y")
...
asmi.annual.2 %>%
  ...
  ggplot(  aes(Prev12, Value, color = Site, group = Site)) +
    geom_point()+
    stat_smooth()+
    facet_grid(Creek~Variable, scales = "free_y")  
...
## With PRISM by hand downloaded
CCc <- asmi.climate %>%
  mutate(Prev12 = ifelse(Month > 7, Year+1, Year)) %>%
  dplyr::distinct(ppt, tmax, tmin, Prev12, Month, Plot, Site, Creek) %>%
  group_by(Creek, Prev12, Site) %>%
  dplyr::summarise(ppt = sum(ppt),
         tmax = mean(tmax),
         tmin = mean(tmin)) %>%
  filter(Prev12 > 2013) %>%
...
SBCc <- asmi.climate %>%
  mutate(Prev12 = ifelse(Month > 7, Year+1, Year)) %>%
  dplyr::distinct(ppt, tmax, tmin, Prev12, Month, Plot, Site, Creek) %>%
  group_by(Creek, Prev12, Site, Plot) %>%
  dplyr::summarise(ppt = sum(ppt),
         tmax = mean(tmax),
         tmin = mean(tmin)) %>%
  filter(Prev12 > 1994) %>%
...
```
Year and site differ for the total alive with AGG. 
...
ggplot(  aes(tmax_springSummer,surv,color = site))+
  ...
  facet_grid(~ppt_winter_bin, scales = "free", 
             labeller = labeller(ppt_winter_bin =
                                   c( "(-1.85,-0.367]" = "low winter precipitation", #
                                      "(-0.367,1.11]" = "mid winter precipitation",
                                      "(1.11,2.59]" = "high winter precipitation"))
             )+
...
Average number of fruit produced per plot
```{r}
asmi.reproductive %>%
  filter(Site %in% c(5,15,26,19)) %>%
  group_by(Fence, Year) %>%
  dplyr::summarise(fruitPerPlot = mean(Total.Fruit),
                   sdPerPlot = sd(Total.Fruit)) %>%
ggplot(   aes(Year, fruitPerPlot, color = Fence)) +
  geom_point(position = position_dodge(width = 0.5)) +
  # geom_errorbar(aes(ymin = fruitPerPlot - sdPerPlot, ymax = fruitPerPlot + sdPerPlot)) +
...
ggplot(  aes(tmax_springSummer,FlYN))+
  ...
  facet_grid(~ppt_winter_bin, scales = "free", 
              labeller = labeller(ppt_winter_bin =
                                    c("(-1.85,-0.277]" = "low winter precipitation",
                                      "(-0.277,1.29]"  = "mid winter precipitation",
                                      "(1.29,2.86]"  = "high winter precipitation"))
              )+
  xlab("Summer maximum temperature")+
...
ggplot(  aes(tmax_springSummer_bin,log(Total.Fruit), color = site))+
  ...
  facet_grid(~ppt_winter_bin, scales = "free", 
             labeller = labeller(ppt_winter_bin =
                                   c("(-1.23,0.137]" = "low winter precipitation",
                                      "(0.137,1.5]"  = "mid winter precipitation",
                                      "(1.5,2.86]"  = "high winter precipitation")))+
...
MPMlamdf %>%
  ...
  dplyr::summarize(lambda = exp(mean(logLambda)),
                   meanLambda = mean(logLambda),
...
p2CC <- ggplot(ps.asmi.dormant[ps.asmi.dormant$Creek == "Cebolla",], 
  ...
  theme(legend.position = "inside",legend.justification.inside = c(1,1), legend.position.inside = c(0.95,0.99),
        legend.background = element_rect(fill=alpha('white', 0.1))) +
  scale_x_continuous(breaks=seq(2015, currentyr-1, 2))
...
# Set wd for prism
prism_set_dl_dir("Q:/Research/All_Projects_by_Species/Astragalus SPECIES/Astragalus_microcymbus/PRISM_asmiclimate")
# prism_set_dl_dir("Q:/Research/All_Projects_by_Species/Astragalus SPECIES/PRISM_data")

#Precipitation total, rain and snow
get_prism_monthlys(type="ppt", mon = 1:12, resolution = "4km", keepZip = FALSE, years =  currentyr) 
get_prism_monthlys(type="ppt", mon = 1:12, resolution = "4km", keepZip = FALSE, years = (currentyr-1):currentyr) 

#Maximum temperature (average)
get_prism_monthlys(type="tmax", mon = 1:12, resolution = "4km", keepZip = FALSE, years = currentyr) # (currentyr-1):

#Minimum temperature (average)
get_prism_monthlys(type="tmin", mon = 1:12, resolution = "4km",keepZip = FALSE, years = currentyr)  # (currentyr-1):

```

Error, didn't download
```{r, eval=FALSE}
get_prism_monthlys(type="ppt", mon = 1, keepZip = FALSE, years = 2025)
