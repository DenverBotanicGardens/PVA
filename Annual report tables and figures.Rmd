...
#Total.Alive - AGG
AGG <- summarySE(ps.asmi, measurevar="Total.Alive", groupvars=c("Fence","Year"))

#Total.Fruit
Fruit <- summarySE(ps.asmi, measurevar="Total.Fruit", groupvars=c("Fence","Year"))

#X..Flowered - %flowered
Repro <- summarySE(ps.asmi, measurevar="X..Flowered", groupvars=c("Fence","Year"))

```


 Figure 8 but different... need to figure out why different    
AGG and Dormant
```{r}
ggplot(AGGDorm[AGGDorm$Total.Plants>0,], aes(Year, Total.Plants, colour = Fence))+
  geom_point(position=position_dodge(0.1), shape=21, fill="white")+
  stat_summary(fun.y = function(x) mean(x, na.rm = TRUE), geom="line")+
  geom_errorbar(aes(ymin=Total.Plants-se, ymax=Total.Plants+se),width=1, position = position_dodge(0.1)) +
  theme_bw() +
...
AGG
```{r}
ggplot(AGG[AGG$Total.Alive>0 & AGG$Year < 2015,], aes(Year, Total.Alive, colour = Fence))+
  geom_point(position=position_dodge(0.1), shape=21, fill="white")+
  stat_summary(fun.y = function(x) mean(x, na.rm = TRUE), geom="line")+
  geom_errorbar(aes(ymin=Total.Alive-se, ymax=Total.Alive+se),width=1, position = position_dodge(0.1)) +
  theme_bw() +
...
Fruit
```{r}
ggplot(Fruit[AGG$Total.Alive>0 & AGG$Year < 2015,], aes(Year, Total.Fruit, colour = Fence))+
  geom_point(position=position_dodge(0.1), shape=21, fill="white")+
  stat_summary(fun.y = function(x) mean(x, na.rm = TRUE), geom="line")+
  geom_errorbar(aes(ymin=Total.Fruit-se, ymax=Total.Fruit+se),width=1, position = position_dodge(0.1)) +
  theme_bw() +
  ylab("Average Fruit per Site")+
  ggtitle("b)") +
  theme(plot.title=element_text(hjust=0))+
  geom_vline(xintercept = 2005.75)+
  geom_vline(xintercept = 2006.75, lty = 4)
