...
2013
```{r}
intra.2013June <- read.csv("Q:/Research/All_Projects_by_Species/Astragalus SPECIES/Astragalus_microcymbus/Raw Data/2013/AsMiJune2013.csv")
...
table(dm$Survey)
dm$Date <- NA
dm$Date <- as.Date("2014-04-21") # All assigned this # LnAprl14  
dm$Date[dm$Survey=="LnMay12"] <- as.Date("2014-05-13") # May1_2014
dm$Date[dm$Survey=="LnJun14"] <- as.Date("2014-06-02") # Jun1_2014
dm$Date[dm$Survey=="LnJun214"] <- as.Date("2014-06-25") # Jun2_2014
dm$Date[dm$Survey=="LnJul14"] <- as.Date("2014-07-15") # Jul1_2014
dm$Date[dm$Survey=="LnAug14"] <- as.Date("2014-08-05") # Aug14_2014
dm$Date[dm$Survey=="LnSep14"] <- as.Date("2014-09-07") # Sep1_2014
dm$Date[dm$Survey=="LnOct14"] <- as.Date("2014-10-08") # Oct1_2014
dm$Date[dm$Survey=="LnOct2814"] <- as.Date("2014-10-28") # Oct2_2014
...
# change to character to remove things that turned out not to be asmi or are saying no cotyledons 
...
#Toothpicks placed when cotyledons were seen
  
toMatch <- c("coty","Coty","new pla","seedli") # ,"tooth")  # toothpick could be there when cotyledons are not
...
dm$Year[grep("15",dm$Survey)] <- 2015
dm$Year[dm$Survey == "LnJune22.24"] <- 2015
...
morethanoneseedling <- lapply(asmicoty, function(i){
  if(nrow(dm[dm$UniqueID==i & dm$status == "seedling" & !is.na(dm$status),])>1){
    dm[dm$UniqueID==i,]
  }
})

lapply(morethanoneseedling[!unlist(lapply(morethanoneseedling,is.null))], function(x) rle(x$status[!is.na(x$status)]))
morethanoneseedling[!unlist(lapply(morethanoneseedling,is.null))][[1]]
...
for(i in asmicoty){
  dm[dm$UniqueID==i,]
  seedlingsurvey <- min(dm$Date[dm$UniqueID==i & !is.na(dm$status) ], na.rm=TRUE) # the first date cotyledon was seen
  dm$status[dm$Date < seedlingsurvey & dm$UniqueID == i] <- NA
# many are nothing because wasn't seen again after cotyledons, warnings are fine, dealt with in the if statement
  ...
  if(i %in% doubleseedlingID){
    dm[dm$Date >= seedlingsurvey & dm$UniqueID==i & dm$status=="seedling" & !is.na(dm$status),]
    dm$status[dm$Date>seedlingsurvey & dm$UniqueID==i & dm$status=="seedling"& !is.na(dm$status)] <- "vegetative"
    }
  ...
  if(is.infinite(lastseen)){
    ...
    # dm[dm$Date>=seedlingsurvey & dm$UniqueID==i & dm$Date<=lastseen,]
    dm$status[dm$Date>seedlingsurvey & dm$UniqueID==i & dm$Date<lastseen & is.na(dm$status)] <- "dormant"
...
dormtags <- unique(dm$UniqueID[dm$status == "dormant" & !is.na(dm$status)])
sort(dormtags)

ggplot(dm[dm$UniqueID %in% dormtags & !is.na(dm$status) & dm$status != "dead",], aes(status, fill = status))+
  geom_bar()+
  facet_wrap(~Date, ncol = 9)+
  theme_bw()+
