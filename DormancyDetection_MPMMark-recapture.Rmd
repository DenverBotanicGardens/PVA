...
MPMlam <- year.var %>%
  ...
  mutate(plot = "All")%>%
  relocate(plot, .after = Year)

lmdas2bind <- lmdas %>%
  relocate(time, .before = estimate) %>%
  dplyr::rename("Year" = time)%>%
  relocate(plot, .after = Year) %>%
  dplyr::select(Year:ucl)

MPMlam %>%
  bind_rows(lmdas2bind) %>%
ggplot(  aes(Year, estimate, color = plot)) +
  ...
  xlab("Year")+
  # scale_color_grey()+
  scale_color_manual(values = c("black", rainbow(16)))+
