...
site2year <- lapply(2015:2021, function(yr){
...
site5year <- lapply(1995:2020, function(yr){
...
stoch.log.lam$Site = c(rep("Site5", length(site5year)),rep("Site15", length(site15year)), 
             rep("Site19", length(site19year)), rep("Site26", length(site26year)))
stoch.log.lam$Year = c(1995:2020, rep(1995:2022, 3))

stoch.log.lam %>%
  filter(Year < 2022) %>%
ggplot(  aes(Year, Approx, color = Site))+
...
# ggplot(MPMlamdf, aes(Year, log(Lambda), color = as.factor(Plot)))+
#   geom_point(position = position_dodge(width = 0.5))+
#   facet_wrap(~site,nrow = 6)+
#   scale_color_manual("Plots", values = rainbow(nrow(SitePlot)))+
#   geom_hline(yintercept = 0)
# 
# ## Some are null
# alllambdas <- do.call(rbind,lapply(MPM.plots, function(m){
#   out <- do.call(rbind,lapply(m, function(x){
#     ifelse(is.null(x), NA, lambda(x))
#   }))
#   outwona <- data.frame(Year = row.names(out), Lambda = out)
#   outwona[complete.cases(outwona),]
# }))
# 
...
multi.proc <- process.data(asmiMulti, begin.time = 1995, model = "Multistrata", groups = "Plot")
...
lmdas <- asmi.models.site$Phi.timePlot.p.dot.f.timePlot$results$derived$`Lambda Population Change` %>%
  bind_cols(Params = params[-length(params)]) %>%
...
allL <- allLambdas %>%
  ...
  filter(plot %in% c("606","607","608","609","610",
                     ...
                     "512","513","514","515",
                     "480","611","614")) %>%
  dplyr::select(c(estimate:ucl, site, Year))
...
PradelMPMsite <- lmdas %>%
  ...
  dplyr::left_join(SitePlot) %>%
  # mutate(site = as.factor(site)) %>%
ggplot(  aes(Year, estimate, color = Method, shape = Method)) +
  geom_point(position = position_dodge(width = 1))+
  geom_errorbar(aes(ymin = lcl, ymax = ucl),position = position_dodge(width = 1))+
  theme_bw()+
  ylab(expression(lambda))+
