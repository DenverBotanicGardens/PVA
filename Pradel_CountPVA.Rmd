...
PVA.lm <- lmer(ySpecies ~ -1 + Year + (1|plot), lmer.df)
...
ggplot(lmdasPlot, aes(time, estimate, color = plot))+
  geom_point()+
  stat_smooth(method = "lm", alpha = 0.3)+
  facet_wrap(~site)+
  theme_bw()+
...
PradelMPMlongplot <- 
  ...
  bind_rows(CountLam) %>%
ggplot(  aes(time, estimate, color = Method, linetype = Method, 
...
ggsave(PradelMPMlongplot, filename = "C:/Users/deprengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter AsMi/Images/lambdasOverTimePradelCountPVA.jpg",
         width=125, height=150,units='mm', dpi=300 )
